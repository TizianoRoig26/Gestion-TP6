...
export function OrderConfirmationPage() {
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        <div className="animate-pulse space-y-4">
          <div className="h-8 bg-surface-tertiary rounded w-48 mx-auto" />
          <div className="h-4 bg-surface-tertiary rounded w-64 mx-auto" />
          <div className="h-32 bg-surface-tertiary rounded-xl mt-8" />
        </div>
      </div>
    );
  }
  ...
  if (isError || !pedido) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <p className="text-text-secondary mb-8">No pudimos encontrar el pedido solicitado</p>
        <Link
          to="/mis-pedidos"
  ...
  function OrderSummary() {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-border-subtle p-6 text-left mb-8">
        <h2 className="font-semibold text-text-primary mb-4">Resumen del pedido</h2>
        ...
        <div className="border-t border-border-subtle pt-3 space-y-1 text-sm">
        ...
        {pedido.direccion_snapshot && (
          <div className="mt-4 pt-4 border-t border-border-subtle">
            ...
            <p className="text-sm text-text-primary">{pedido.direccion_snapshot}</p>
          </div>
        )}
      </div>
    );
    ...
    return (
      <div className="flex flex-col sm:flex-row gap-4 justify-center">
        <Link
          ...
          className="bg-primary-500 text-white px-8 py-3 rounded-xl font-medium hover:bg-primary-600 transition-colors"
        >
          Ver detalle del pedido
        </Link>
        <Link
          to="/catalogo"
          className="bg-white text-text-primary px-8 py-3 rounded-xl font-medium border border-border-default hover:bg-surface-secondary transition-colors"
        >
          Seguir comprando
        </Link>
      </div>
  ...
  if (esMercadoPago && (loadingPago || !pago)) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <h1 className="text-2xl font-bold text-text-primary mb-2">Procesando pago...</h1>
        ...
        {/* Polling indicator */}
        <div className="flex items-center justify-center gap-2 text-xs text-text-tertiary mb-8">
  ...
  if (esMercadoPago && pago?.mp_status === "approved") {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <h1 className="text-2xl font-bold text-text-primary mb-2">¡Pago aprobado!</h1>
        <p className="text-text-secondary mb-8">
          Tu pago para el pedido <span className="font-semibold text-text-primary">#{pedido.id}</span>{" "}
          fue procesado exitosamente
        </p>

        {/* Payment reference */}
        {pago.mp_payment_id && (
          <div className="bg-primary-50 border border-primary-100 rounded-lg p-4 mb-8 inline-block">
            <p className="text-xs text-primary-500 uppercase tracking-wider font-medium mb-0.5">
              ID de transacción
            </p>
            <p className="text-sm text-primary-700 font-mono font-semibold">
              {pago.mp_payment_id}
            </p>
          </div>
        )}
  ...
  if (esMercadoPago && pago?.mp_status === "rejected") {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <h1 className="text-2xl font-bold text-text-primary mb-2">Pago rechazado</h1>
  ...
  if (esMercadoPago && pago?.mp_status === "pending") {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <h1 className="text-2xl font-bold text-text-primary mb-2">Pago pendiente</h1>
        <p className="text-text-secondary mb-8">
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-16 text-center">
      ...
      <p className="text-text-secondary mb-1">
        Tu pedido <span className="font-semibold text-text-primary">#{pedido.id}</span> fue creado exitosamente
      </p>
      <p className="text-sm text-text-tertiary mb-8">Pagás al recibir el pedido</p>
