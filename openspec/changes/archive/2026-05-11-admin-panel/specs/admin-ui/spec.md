...
- **THEN** el sidebar solo muestra: Dashboard (limitado), Catálogo, Stock
- **WHEN** un usuario con rol PEDIDOS accede a `/admin/*`
- **THEN** el sidebar solo muestra: Dashboard (limitado), Pedidos

#### Scenario: Sidebar responsive
- **WHEN** la pantalla es menor a 768px
- **THEN** el sidebar se colapsa como overlay con un botón hamburguesa para abrirlo/cerrarlo
...
- **THEN** se abre un modal/formulario con datos del usuario
- **THEN** puede cambiar nombre, apellido, email
- **THEN** puede cambiar el rol (selector)
- **THEN** al guardar, se muestra toast de éxito

#### Scenario: Desactivar/Activar usuario
- **WHEN** un ADMIN hace clic en "Desactivar" en un usuario activo
- **THEN** se muestra un modal de confirmación "¿Estás seguro de desactivar este usuario?"
...
- **THEN** ve una tabla con: ID, cliente, estado (badge color), total, fecha, forma de pago
...
- **THEN** se abre un selector con solo los estados válidos según la FSM
- **THEN** si es CANCELADO, se solicita motivo obligatorio
...
- **THEN** ve el detalle completo: items, historial de estados (timeline), pago, dirección
...
- **THEN** ve una tabla con: producto, categoría, stock actual, disponible (toggle), precio
- **THEN** el stock en rojo si es menor a 5
...
- **THEN** al confirmar, se actualiza vía PATCH /api/v1/productos/{id}

#### Scenario: Toggle disponibilidad
- **WHEN** un ADMIN o Gestor de Stock hace clic en el toggle de disponible
- **THEN** se actualiza vía PATCH /api/v1/productos/{id}/disponibilidad
...
- RN-AU03: Los cambios de estado de pedido validan la FSM (mismos rules que backend)
